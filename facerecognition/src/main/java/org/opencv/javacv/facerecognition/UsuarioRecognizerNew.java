...
public  class UsuarioRecognizerNew {
    ...
    public UsuarioRecognizerNew(Usuario usuario) {
	...
	public boolean train() {
        ...
        MatVector images = new MatVector(usuario.getFaces().size());

        int[] labels = new int[usuario.getFaces().size()];
        ...
        for (byte[] face : usuario.getFaces()) {

			Bitmap bitmap;
			label = usuario.getId();

			bitmap = BitmapFactory.decodeByteArray(face, 0, face.length);
